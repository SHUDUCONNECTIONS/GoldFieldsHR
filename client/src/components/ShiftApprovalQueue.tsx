...
import { ShiftTypeLabels, type ShiftChangeRequestDto } from "../types/workShift";

interface ShiftApprovalQueueProps {
  title: string;
  items: ShiftChangeRequestDto[];
  isBusy: boolean;
  onApprove: (id: string) => void;
  onReject: (id: string, reason: string) => void;
}

export function ShiftApprovalQueue({ title, items, isBusy, onApprove, onReject }: ShiftApprovalQueueProps) {
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-slate-600">
                    {formatDate(item.requestedShiftDate)} — {ShiftTypeLabels[item.requestedShiftType]} shift
                  </p>
                  <p className="mt-1 text-xs text-slate-500">Reason: {item.reason}</p>
                  {item.comments && <p className="text-xs text-slate-500">Comments: {item.comments}</p>}
                </div>
